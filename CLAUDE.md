...
- A `lib/` doc comment is published — the module's header block and the first paragraph of each function's comment are what `docs/libraries.md` prints. For `ai/lib/` (Python), only the module docstring is published into `docs/ai-libraries.md`; per-function docstrings are not rendered. Keep the first paragraph to the contract, including what each argument does, and put implementation rationale below a blank comment line (or below a blank line in a Python docstring).
...
- **`ai/lib/` is a layer stack** — each package declares its layer and the exact set of sibling packages it may import on the first line of its `__init__.py` (`"""Layer 4 — PR state and lifecycle. May import: core, config, git, gh, agent."""`). That sentence is the only place a package's layer is written down: the validator parses it rather than carrying a table of its own, so the layer is stated once, in the file whose contents it governs. The permitted set is narrower than the number — `gh` and `agent` both sit at layer 3 and neither may import the other — and a package that declares no layer is itself a failure, which is what keeps a new one from joining the tree unplaced. Run `bin/local/validate-ai-layers` with no arguments to print the current stack (enforced by `bin/local/validate-ai-layers`).
...
- **`ai/pi/extensions/` holds extensions that load in every Pi session** — enumerated with the same `*/` glob and symlinked into `~/.pi/agent/extensions/` by `ai/pi/steps.sh`, so any subdirectory added here loads globally, in sessions with no repository and no environment set up for it. An extension only some runs should load goes in `ai/pi/extensions-cli/`, which is passed with `--extension` and never installed.
- **A behavioural guard is written twice, once per harness** — `ai/claude/bin/claude-bash-guard` is a PreToolUse hook and does not run under Pi; a `ai/pi/extensions/<name>/` extension is a `tool_call` handler and does not run under Claude Code. A rule that must hold everywhere needs both, sharing one threshold constant with a test that fails when they drift (`sleep-guard` is the worked example). Put the predicate in a sibling module importing nothing — the entry point imports the Pi SDK as a value, so only a module without it can be loaded by `node` in a test.
