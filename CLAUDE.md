...
- **`ai/guidelines/rules/` holds rules and nothing else** — `resolve_layers` enumerates it with a `*.md` glob and `claude-rules sync` symlinks the result into `~/.claude/rules/`. `step_pi_guidelines` reads that installed directory — the output of the sync, not `resolve_layers`' return value, which is why it also sees the machine-local rules that exist nowhere else — and concatenates the always-on ones into `~/.pi/agent/AGENTS.md`. A note or a template dropped here is installed into both harnesses as a rule. Which harnesses a rule reaches is decided by its own frontmatter — see `rules-authoring.md` § Which harnesses a rule reaches.
