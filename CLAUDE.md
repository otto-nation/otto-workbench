...
- **`ai/guidelines/rules/` holds rules and nothing else** — `resolve_layers` enumerates it with a `*.md` glob and two consumers read the result: `claude-rules sync` symlinks each file into `~/.claude/rules/`, and `step_pi_guidelines` concatenates the always-on ones into `~/.pi/agent/AGENTS.md`. A note or a template dropped here is installed into both harnesses as a rule. Which harnesses a rule reaches is decided by its own frontmatter — see `rules-authoring.md` § Which harnesses a rule reaches.
