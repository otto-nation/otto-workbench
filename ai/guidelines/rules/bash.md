...
- The same holds for any temporary state a script grants itself — an elevated database role, say, or more generally a disabled guard or a stashed config. Register the teardown on `EXIT` when the state is taken, not as a trailing command: under `set -e` the trailing form is skipped by exactly the failures that make cleanup matter. Clear it with `trap - EXIT` after an explicit teardown so it does not run twice. This applies to scripts embedded in templates (Helm, Ansible, CI YAML), which is where the trailing form usually survives review
