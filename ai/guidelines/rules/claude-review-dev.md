...
`otto-log query --pr <n>`. When a record carries a `log:` line, the whole of
what the failing command printed is in
`~/.local/state/workbench/trail/artifacts/<YYYY-MM>/` — the record holds only
