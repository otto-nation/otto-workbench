...
- Validate an input where it enters, not where it is used. A guard reached only on one branch means validation is decided by the data rather than by the contract — the same call passes unchecked whenever an earlier value sends it down the other path, and an early return for a legitimate case (a grace period, an empty set) silently skips every cross-field check that sat below it. Check the whole input contract at the top, before the first branch or early return
