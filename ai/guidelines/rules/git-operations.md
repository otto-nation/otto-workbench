...
- Always use `task --global pr:create -- --no-issue --draft` to create a PR — `task pr:create` does not exist (no local target), and omitting `-- --no-issue` blocks on an interactive issue-number prompt
- Always open PRs as drafts (`--draft`) — all PRs should go through review before being marked ready
