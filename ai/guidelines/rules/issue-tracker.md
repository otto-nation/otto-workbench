...
- When no such line is in your context, the harness you are running in has no SessionStart hook — that is Claude Code's, and the absence says nothing about the repo. Ask the same question of the same source directly: `otto-workbench config get issue_tracker.provider`, which prints the scope it resolved from, the value, and the file holding it. No value is the unconfigured case below, not a reason to guess
