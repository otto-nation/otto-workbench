...
- The SessionStart context line answers this: a configured repo shows `Issue tracker: {provider}` (e.g. `Issue tracker: github`); an unconfigured one shows `Issue tracker: not configured — ask before filing, then record the answer by running otto-workbench config set issues.provider PROVIDER`. Use it — do not infer a tracker from a branch name, a CLI that happens to be installed, or another repo's convention
- When no such line is in your context, the harness you are running in has no SessionStart hook — that is Claude Code's, and the absence says nothing about the repo. Ask the same question of the same source directly: `otto-workbench config get issues.provider`, which prints the scope it resolved from, the value, and the file holding it. No value is the unconfigured case below, not a reason to guess
- When it reads unconfigured, ask before filing, then record the answer with `otto-workbench config set issues.provider PROVIDER` — add `--project` to record it in this repo's `.workbench.yml` instead of for every repo on the machine. Never pick one silently
...
- For a repo other than the one this session is in, the Project Registry table in `~/.claude/machine/machine.md` names each registered repo's tracker in its `Issues` column, resolved through the same scopes the SessionStart line uses when the profile was written. A bare value is the repo's own answer, from its checkout or from the file above its worktrees. A value tagged `(global)` is one it only inherits from the machine — usable, but the repo has not said so itself, so record it with `--project` or `--container` if you are working there and want it pinned. A row reading `unset` is a repo that has never declared one at any scope — it is a question still owed, not a repo without a tracker
...
- An issue filed as follow-up work — deferred from a PR, split out of a plan's out-of-scope section, or carved off an issue whose scope grew — carries the labels in `issues.labels`, which is `follow-up` unless the repo says otherwise. Read the resolved list with `otto-workbench config get issues.labels`; a repo that has opted out spells it `labels: []` and you label nothing
