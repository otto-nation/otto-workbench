...
9. If the merged findings contain a ## Prior findings section, copy every line of it into your output unchanged — it records which prior findings the group agents found fixed, and it is stripped before the review is published. Add lines to it only where a section below asks you to
