...
def post(repo: str, pr_number: str, review_file: str, submit: bool, *, bin_dir: Path) -> None:
...
def submit_pending(repo: str, pr_number: str, review_file: str) -> None:
    """Submit a review that was posted as pending.
...
def resolve(
    ...
    *,
    ...
    if auto_post:
        post(repo, pr_number, str(review_file), auto_submit, bin_dir=bin_dir)
    ...
    if not prompt.confirm("Satisfied with the review?"):
        return PostResult(False, False, "", (
            ...
            "Post to GitHub:   pr review --post",
        ))

    if not prompt.confirm("Post review to GitHub?"):
        return PostResult(False, False, "", (_HINT_POST,))

    post(repo, pr_number, str(review_file), False, bin_dir=bin_dir)

    submitted = False
    if prompt.confirm("Submit review now?"):
        submit_pending(repo, pr_number, str(review_file))
        submitted = True
