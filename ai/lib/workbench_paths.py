...
def projects_registry() -> Path:
    ...
    backfill and the CLI, and this side holds the writes the Python tools make.
    ``workbench_projects`` is where those reads and writes live; this function
    only names the path, so the two languages cannot disagree about it.
    """
