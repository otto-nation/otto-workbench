...
# shared predicate.
_pi_rule_reaches_pi() {
  [[ -n "$(frontmatter_field "$1" paths)" ]] && return 1
...
#
# ceiling: Pi's context depends on Claude's rules having synced first, which
# ai/setup.sh's glob order and otto-workbench sync's component order both
# happen to give. Upgrade to a harness-neutral installed rules root if a
# Pi-only machine ever needs this, or once Pi grows a rules directory of its
# own.
step_pi_guidelines() {
  ...
  while IFS= read -r file; do
    _pi_rule_reaches_pi "$file" && included+=("$file")
