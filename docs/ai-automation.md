...
`eval_task` because its calls need `env=`, which the client does not take — it runs
them through `proc.run`, which does. A new call site should still go through the
client.
