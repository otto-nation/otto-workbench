...
**Project-scoped migrations:** a migration that has to visit every repo on the machine reads the [project registry](libraries.md#projectssh) rather than scanning for `.claude` directories under a list of likely git roots. Registration is an observation, so it can only ever be late, and a late one does not reopen a migration already recorded as applied — for a repo that arrives after a migration has run, the fix is `otto-workbench projects add` before the next sync, or running the migration's effect by hand. This is the cost of not scanning, and it buys correctness in the common case: the old scan silently missed repos it had no way to know about and reported success anyway.
