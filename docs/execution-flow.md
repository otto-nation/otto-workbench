...
Registration is an observation, so it can only ever be late, and a late one does not reopen a migration already recorded as applied. Backfill covers the repos that predate the registry; for one that arrives after a migration has run — a fresh clone, or a repo whose first workbench command came later — the fix is `otto-workbench projects add` before the next sync, or running the migration's effect by hand. This is the cost of not scanning, and it buys correctness in the common case: the old scan silently missed repos it had no way to know about, and reported success anyway.
