...
| `project_repo_worktrees` | every registered work tree, under the repo it belongs to. Prints `<repo id><TAB><work-tree path>` — the reverse of a registry line, which is `<path><TAB><id>` — with a repo's work trees consecutive, repos in the order the registry first names one of theirs and each repo's work trees in registry order. |
