...
| `install_via_installer CMD URL LABEL` | installs LABEL by running the vendor's own install script at URL when CMD is not already in PATH, returning non-zero with a pointer to URL when curl is missing or the installer fails. |
