...
| `resolve_rules RESULT_NAMEREF` | Merges the three rule layers into an associative array: basename -> source path. Later layers win for a same-named file: repo defaults, then the operator's override layer, then what workbench-rules generated for this machine. RESULT_NAMEREF must be a declared associative array in the caller. |
