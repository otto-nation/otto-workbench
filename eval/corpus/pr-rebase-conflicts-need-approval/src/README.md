...
shim is passthrough), so `src/auth.py` and `tests/test_auth.py` are present to
keep what `git status`/`git log` shows consistent with what `responses.json`'s
stubbed `pr rebase` output narrates as conflicted.
