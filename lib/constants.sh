...
# Cooldown stamps for the Stop-hook gates. Most are per repo, one file per repo
# named by _canonical_slug; RETRO_STAMP_FILE below is the exception and is one
# per machine. Under the state root rather than in a harness's tree because the
# gates ask a question about a repo, not about a harness: a repo worked in only
