...
import pr_state
from review_common import plural, render_status, resolve_review_verdict
...
def test_resolve_verdict_derives_from_counts_when_prose_is_silent(tmp_path):
    ...
    assert resolve_review_verdict(review) is pr_state.ReviewVerdict.NEEDS_DISCUSSION
...
def test_resolve_verdict_prose_cannot_under_report_blocking_findings(tmp_path):
    ...
    assert resolve_review_verdict(review) is pr_state.ReviewVerdict.CHANGES_REQUESTED
...
def test_resolve_verdict_disapprove_survives_any_counts(tmp_path):
    ...
    assert resolve_review_verdict(review) is pr_state.ReviewVerdict.DISAPPROVE
...
def test_resolve_verdict_self_review_still_reports_disapprove(tmp_path):
    ...
    review = _review(tmp_path, "## Verdict\nDisapprove — wrong approach.\n")
    assert resolve_review_verdict(review, self_review=True) is pr_state.ReviewVerdict.DISAPPROVE
